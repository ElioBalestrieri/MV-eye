...
out_feat_path          = '/remotedata/AgGross/TBraiC/MV-eye/STRG_computed_features/TimeFeats_bandpassed';
...
parfor isubj = 1:nsubjs
    ...
    for icond = 1:2
        ...
        mdls_codes = {'delta', 'theta', 'alpha', 'beta', ...
        ...
        for imdl = 1:length(mdls_codes)
            ...
            bandRange = cfg_feats.freqRanges.(mdl_name)
            ...
            F = mv_wrap_catch22(cfg_feats, dat_bp, F);
            F = mv_periodic_aperiodic(cfg_feats, dat_bp, F);

            % append cfg for easy check the operations performed before
            F.cfg_feats = cfg_feats;

            % save
            fname_out_feat = [subjcode, '_' this_cond '_' mdl_name '_TimeFeats.mat'];
