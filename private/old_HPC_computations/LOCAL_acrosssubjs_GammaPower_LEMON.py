...
for imdl in mdltypes:
    ...
    feat_names = list(filter(lambda k: 'Hz' in k, fullX_train.keys()))
            
    # preallocate matrix (if first iteration on the loop)
    mat_accs = np.empty((1, len(feat_names)))
    ...
    DF = pd.DataFrame(data=mat_accs, columns=feat_names, index=[imdl])
...
plt.figure()
sns.barplot(DF.T.sort_values(ascending=False, by=imdl).T, orient='h')
plt.tight_layout()
