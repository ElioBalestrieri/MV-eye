...
samples_pre = run_experiment(cfg, diffusion, test_data_set, train_data_set)

savemat(data_path + "/orig_sample_100epochs_pre.mat", {"samples":np.array(train_data_set.dataset.data_array)})
savemat(data_path + "/gen_sample_100epochs_pre.mat", {"samples":samples_pre})
...
samples_post = run_experiment(cfg, diffusion, test_data_set, train_data_set)

savemat(data_path + "/orig_sample_100epochs_post.mat", {"samples":np.array(train_data_set.dataset.data_array)})
savemat(data_path + "/gen_sample_100epochs_post.mat", {"samples":samples_post})

#%% compare spectra

ntrl, nchan, ntp = samples_post.shape
T = 1/ntp # sfreq == ntp, 256 Hz
xf = fftfreq(ntp, T)[:ntp//2]

out_post = np.array(samples_post)
out_pre = np.array(samples_pre)

fft_pre = fft(out_pre, axis=2)
fft_post = fft(out_post, axis=2)

avg_pre = np.mean(2/ntp * np.abs(fft_pre[:, :, 0:ntp//2]), axis=0)
avg_post = np.mean(2/ntp * np.abs(fft_post[:, :, 0:ntp//2]), axis=0)

plt.figure()
plt.plot(np.log10(xf[1:]), np.log10(avg_pre[42, 1:]))
plt.plot(np.log10(xf[1:]), np.log10(avg_post[42, 1:]))
