...
# def recursiveScaler(X, groups):
    
#     ncvs = np.unique(groups)
    
#     # # shuffle group order to avoid that the across subjects the tested group is 
#     # # consistently at early (or late) eyperiment stage
#     tmp_labels = np.arange(len(ncvs)); np.random.shuffle(tmp_labels)
    
#     shuffled_groups = np.zeros(groups.shape)
    
#     for igroup in ncvs:
#         shuffled_groups[groups==igroup] = tmp_labels[int(igroup)]
...
#     #     # te_ = X[shuffled_groups==igroup, :]
#     #     # tr_ = X[shuffled_groups!=igroup, :]
        
#     #     te_ = X[groups==igroup, :]
#     #     tr_ = X[groups!=igroup, :]
   
#     #     te_ = remove_nan.fit_transform(te_)
#     #     tr_ = remove_nan.fit_transform(tr_)
        
#     #     tmp_scaler = RobustScaler().fit(tr_)
#     #     out_te_ = tmp_scaler.transform(te_)
        
#     #     out_dat.append(out_te_)
        
#     # scaled_X = np.concatenate(out_dat, axis=0)
    
#     return X, groups # scaled_X
...
def MyDecoders(X, Y, groups, ifeat, imdl, icond):
    ...
    LDA_acc = cross_val_score(LDA_pipeline, X, Y, cv=LeaveOneGroupOut(), 
                              groups=groups,
                              scoring='balanced_accuracy')

    LogReg_acc = cross_val_score(LogReg_pipeline, X, Y, cv=LeaveOneGroupOut(), 
                                 groups=groups,
                                 scoring='balanced_accuracy')

    tmp_acc = list(SVM_acc) + list(LDA_acc) + list(LogReg_acc)
    tmp_feat = [ifeat]*len(tmp_acc)
    tmp_class = ['SVM']*n_cvs + ['LDA']*n_cvs + ['LogReg']*n_cvs
    tmp_cond = [icond]*len(tmp_acc)
    tmp_mdl = [imdl]*len(tmp_acc)
    tmp_fold = [f'{i}CVpart' for i in range(n_cvs)]*3
...
for icond in expconds:
    ...
    for imdl in mdltypes:
        ...
        for ifeat in featnames:
            ...
            stable_DF.to_csv(outfold + 'across_accuracy_TMP.csv', index=False)
        ...
        stable_DF.to_csv(outfold + 'across_accuracy_FINAL.csv', index=False)
